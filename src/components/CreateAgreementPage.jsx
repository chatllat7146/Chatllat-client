...
export default function CreateAgreementPage() {
    ...
    const handleGenerateLink = async () => {
        ...
        try {
            ...
            const response = await fetch(
                `${import.meta.env.VITE_BACKEND_URL}/api/agreement`,
                {
                    method: "POST",
                    headers: {
            ...
            if (result.success) {
                ...
                setGeneratedLink(result.newShareLink);
    ...
    // Handle PDFs specially - upload as image for better viewing
    if (fileType === 'application/pdf' || extension === 'pdf') {
      return 'image'; // This allows PDF viewing and transformation
    }

    // Video files
    if (fileType.startsWith('video/') || 
        ['mp4', 'avi', 'mov', 'wmv', 'flv', 'webm', 'mkv', 'm4v', '3gp', 'ogv'].includes(extension)) {
      return 'video';
    }
    
    // Image files
    if (fileType.startsWith('image/') || 
        ['jpg', 'jpeg', 'png', 'gif', 'svg', 'bmp', 'webp', 'ico', 'tiff', 'tif', 'heic', 'heif'].includes(extension)) {
      return 'image';
    }
    
    // All other files (documents, JSON, etc.) should use 'raw'
    return 'raw';
  };
    ...
    const handleUploadFiles = async () => {
        ...
        try {
            for (let i = 0; i < uploadedFiles.length; i++) {
                ...
                console.log(
                    `Uploading file ${i + 1}/${uploadedFiles.length}: ${
                        file.name
                    }`
                );
                ...
                formDataUpload.append("upload_preset", "chatllat");
                ...
                if (isPDF) {
                    ...
                    formDataUpload.append("format", "pdf");
                    formDataUpload.append("pages", "true");
                } else if (resourceType === "raw") {
                    formDataUpload.append("resource_type", "raw");
                }

                const response = await axios.post(
                    `https://api.cloudinary.com/v1_1/djsgei5zh/${resourceType}/upload`,
                    formDataUpload,
                    {
                        headers: {
                ...
                let finalUrl = response.data.secure_url;
                if (isPDF) {
                    finalUrl = response.data.secure_url.replace(
                        "/upload/",
            ...
            let message = `Uploaded ${successfulUploads.length} file(s) successfully`;
            if (failedCount > 0) {
            ...
            setNotification({
                open: true,
                message: message,
                severity: successfulUploads.length > 0 ? "success" : "error",
            });
        } catch (err) {
